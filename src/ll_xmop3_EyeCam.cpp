...
/*! LL_CAM_HAS_LCD -- does THIS board have the ST7789 panel this driver also drives?
    The S3-EYE does; the Freenove WROVER-CAM / 4WD carry the same OV2640 with NO panel at all.
    The two halves were fused because only one board had ever used the file.  Compiling the LCD in
    on a board that has none is not merely dead weight: its pins (43/44/47/48) DO NOT EXIST on a
    classic ESP32, which has GPIO 0-39, so it would configure absent GPIOs and read as a
    camera-capable build. */
#ifndef LL_CAM_HAS_LCD
  #if LL_ESP32S3
    #define LL_CAM_HAS_LCD 1
  #else
    #define LL_CAM_HAS_LCD 0
  #endif
#endif
...
#endif  // LL_CAM_HAS_LCD -- LCD pin block
...
#if LL_CAM_HAS_LCD   // ---- panel-only region: absent on the Freenove WROVER-CAM / 4WD
...
#if LL_CAM_HAS_LCD
      //! Panel mops exist only where there IS a panel.  On a board without one these names stay
      //! UNBOUND rather than being bound to something that silently does nothing -- an unbound name
      //! tells a caller immediately, a no-op stub makes a missing panel look like a working one.
